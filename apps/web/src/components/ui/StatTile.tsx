...
interface StatTileProps {
  ...
  progress?: number
  change?: ReactNode
  changeTone?: 'acc' | 'ok' | 'warn' | 'muted'
  variant?: 'card' | 'card-2'
  size?: 'sm' | 'md'
  className?: string
}

const TONES: Record<NonNullable<StatTileProps['changeTone']>, string> = {
  acc: 'var(--acc)',
  ok: 'var(--ok)',
  warn: 'var(--warn)',
  muted: 'var(--fg-3)',
...
export default function StatTile({
  ...
  return (
    <div className={`${variant} ${className}`.trim()} style={{ padding }}>
      <div
        className="mono"
        style={{
          fontSize: labelSize,
      ...
      <div
        className="row mt-1.5 items-baseline"
        style={{ gap: 4 }}
      >
        <span
          className="serif"
