...
export default function ClientFeedbackPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <h1 className="text-2xl font-bold text-[var(--fg)] mb-6">Check-ins — {client?.full_name || 'Client'}</h1>
      ...
      {showCreate && (
        <div className="card p-6 mb-6">
          <h2 className="text-lg font-bold text-[var(--fg)] mb-4">Send a Check-in Now</h2>
          ...
          <label className="block text-sm font-medium text-[var(--fg-2)] mb-2">Questions</label>
          {questions.map((q) => (
          ...
          <div className="flex gap-3">
            <button onClick={() => setShowCreate(false)}
              className="px-6 py-2 border border-[var(--line-strong)] rounded-lg text-sm font-medium text-[var(--fg-3)] hover:bg-[var(--ink-2)]">
              Cancel
            </button>
      ...
      {requests.length === 0 && !showCreate ? (
        <div className="card p-12 text-center">
          <Clock className="h-12 w-12 text-[var(--fg-4)] mx-auto mb-3" />
          <h3 className="font-semibold text-[var(--fg)] mb-1">No check-ins yet</h3>
          <p className="text-sm text-[var(--fg-3)] mb-4">Send a check-in or set up a recurring schedule.</p>
          <button onClick={() => setShowCreate(true)}
            className="text-sm font-medium text-[var(--acc-text)] hover:underline">Send Check-in</button>
        </div>
      ) : (
...
function ScheduleCard({ schedule, templates, trainerId, clientId, onRefresh }: {
  ...
  return (
    <div className="card p-5 mb-6 border-l-4 border-l-[var(--acc)]">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          <Calendar className="h-5 w-5 text-[var(--acc-text)]" />
          <h3 className="font-semibold text-[var(--fg)]">Recurring Schedule</h3>
        </div>
        {!editing && (
          <button onClick={() => setEditing(true)}
            className="text-sm font-medium text-[var(--acc-text)] hover:underline">
          ...
          <div className="flex items-center gap-3 mt-3">
            <button onClick={toggleActive}
              className={`text-xs font-medium px-3 py-1.5 rounded-full ${
                schedule.is_active ? 'bg-[var(--success-bg)] text-[var(--ok-text)]' : 'bg-[var(--ink-2)] text-[var(--fg-3)]'
              }`}>
              {schedule.is_active ? 'Active' : 'Paused'}
            </button>
            <button onClick={() => setEditing(true)} className="text-xs text-[var(--fg-4)] hover:text-[var(--fg-2)]">Edit</button>
            <button onClick={handleDelete} className="text-xs text-[var(--fg-4)] hover:text-[var(--danger-text)]">Remove</button>
          </div>
        </div>
      ) : editing ? (
        <div className="space-y-3 mt-3">
          ...
          <div className="flex gap-2 pt-2">
            <button onClick={() => setEditing(false)}
              className="px-4 py-2 text-sm font-medium text-[var(--fg-3)] border border-[var(--line-strong)] rounded-lg hover:bg-[var(--ink-2)]">
              Cancel
            </button>
          ...
          <p className="text-xs text-[var(--fg-4)]">
            ...
            <Link href="/check-ins" className="text-[var(--acc-text)] hover:underline ml-1">Manage templates →</Link>
          </p>
        </div>
      ) : (
        <p className="text-sm text-[var(--fg-4)]">No recurring schedule. Click &ldquo;Set up&rdquo; to auto-send check-ins.</p>
      )}
    </div>
  )
}
  ...
  return (
    <div className="card p-5">
      <button onClick={() => request.status === 'completed' && setExpanded(!expanded)}
        ...
        <div className="flex items-center justify-between mb-2">
          <h3 className="font-semibold text-[var(--fg)]">{request.title}</h3>
          <div className="flex items-center gap-2">
            ...
            {request.status === 'completed' && (
              expanded ? <ChevronDown className="h-4 w-4 text-[var(--fg-4)]" /> : <ChevronRight className="h-4 w-4 text-[var(--fg-4)]" />
            )}
          </div>
        </div>
      </button>
      ...
      {expanded && request.status === 'completed' && request.responses && (
        <div className="border-t border-[var(--line)] mt-3 pt-3 space-y-3">
          {request.questions.map((q: FeedbackQuestion, i: number) => {
            ...
            return (
              <div key={q.id}>
                <p className="text-xs font-medium text-[var(--fg-3)]">{q.question}</p>
                {q.type === 'photo' && Array.isArray(resp?.answer) ? (
                  ...
                  <p className="text-sm text-[var(--fg)] mt-0.5 font-semibold">{resp?.answer ?? '—'} / {q.type === 'rating' ? 5 : 10}</p>
                ) : (
                  <p className="text-sm text-[var(--fg)] mt-0.5">{String(resp?.answer ?? '—')}</p>
                )}
              </div>
            )
          })}
