...
| F42 | 2026-07-16 | PASS | assign-intake.spec.ts (new): F40 remains PASS and F60 remains at its prior BLOCKED-with-canonical-tracking state (PR #75 open + issue #74; no drift this run — still waiting on human review/Stripe secrets, not something QA can unblock), so this run followed the charter's next step and covered F42, the next never-checked multi-user PT↔client flow. Linked a seeded coach+client directly (`linkClientToTrainer`, same helper `createLinkedPair` uses) and added a new `seedCustomIntakeQuestion()` helper (`apps/web/e2e/lib/seed.ts`) to seed one active, required custom intake question for the coach — bypassing the `/coach-questions` UI, same pattern as other seed helpers. Client then drove the real 10-step onboarding wizard (`MANAGED_CLIENT_STEPS`, gated on `coachCustomQuestions.length > 0`), answering the coach's question on the extra "Coach Questions" step, through to the review step and "Submit intake to coach" (managed clients get this button instead of the free-client "Go to Dashboard" — same `handleFinish('dashboard')` handler, discovered and handled in this run). Coach (separate browser context, logged in concurrently) then opened the real `/clients/<id>` page and saw the "Custom intake answers" section render the client's exact question label and answer text. 3/3 green (initial run + 2 repeats), required `npm run build && npm run start` (CI mode) per the suite's established cold-start-flake precedent. |
