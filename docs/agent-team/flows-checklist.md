...
| F05 | 2026-07-12 | PASS | Priority list (F40-F61) remains resolved. F02/F03/F05 were tied oldest at 2026-07-04; F02 was re-verified earlier today (this file) and F03 was re-verified in a separate concurrent run (open PR #121, not yet merged, so not duplicated here) — picked F05, the last of the tied three. Re-ran `smoke.spec.ts` via the deterministic suite: "logged-out users are kept out of the app" (`/dashboard` → `/login` redirect for an unauthenticated visitor) passes, plus all 10 other smoke tests in the same spec (11/11 green). Also re-ran the F05 test in isolation (`-g "kept out"`) for a second confirmation (1/1 green). No code changes needed — pure re-verification. `npm run lint` and `npm run build` both pass (pre-existing warnings only). Synthetic test users cleaned up after the run. F60 not re-attempted: PR #75 + tracking issue #74 remain open/canonical, no new drift. |
