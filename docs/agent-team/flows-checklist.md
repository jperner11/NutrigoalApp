...
| F60 | 2026-07-07 | BLOCKED | billing-upgrade.spec.ts (new): /pricing renders paid-tier CTAs; clicking "Choose unlimited" as a free client calls POST /api/stripe/checkout with the right plan and follows the returned session URL (3/4 runs green — one transient webServer cold-start flake, self-resolved on retry). No STRIPE_SECRET_KEY/STRIPE_WEBHOOK_SECRET/STRIPE_PRICE_* in e2e/CI (only E2E_SUPABASE_* in e2e.yml), so a real Stripe Checkout session and a real signed webhook can't be created — the spec mocks both the checkout API and the destination redirect (never navigates to real checkout.stripe.com). The actual tier-flip (checkout.session.completed webhook → user_profiles.role) is unverified end-to-end. GitHub issue filed for the missing Stripe test-mode secrets gap (label: qa). |
